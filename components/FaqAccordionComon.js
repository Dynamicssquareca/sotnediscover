...
const FaqAccordionComon = ({ faqList = [], categoryData = null }) => {
  // open state for accordion (keeps your original behavior)
  ...
  // If parent passed categoryData with faqs, prefer that.
  // Otherwise use faqList prop. If both empty/invalid, use defaultFaqs.
  const categoryFaqs = Array.isArray(categoryData?.faqs) ? categoryData.faqs : null;

  // normalize input: prefer categoryFaqs -> faqList -> defaultFaqs
  let candidateFaqs = categoryFaqs ?? faqList ?? [];

  // filter & normalize each item so we always have title & description strings
  const validFaqList = candidateFaqs
    .map((item) => {
      if (!item) return null;
      // the API items are usually { title, description }
      const title = (item.title || item.question || '').toString().trim();
      const description = (item.description || item.answer || '').toString().trim();
      if (!title || !description) return null;
      return { title, description };
    })
    .filter(Boolean);

  const items = validFaqList.length > 0 ? validFaqList : defaultFaqs;
...
export default FaqAccordionComon;
